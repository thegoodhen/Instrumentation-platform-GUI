 ...
 * @author thegoodhen
 * @param <T> the type this factory is going to create
 */
public abstract class AbstractContainerFactory<T extends Container>{
	...
	int resolveTag(int tagNumber) {
		//AbstractContainerFactory cf=new AbstractContainerFactory<Register>();
		//Container newCont=cf.create(xmlAsBytes, index);
		//cont.addContainer(newCont);
		return AbstractContainerFactory.getNewIndex();
	}
...
{
	AbstractContainerFactory.xmlAsBytes=xml;
}
...
public final void setContainer(Container cont)
{
	...
	{
		AbstractContainerFactory.startIndex=index;
	}
}
